import { uploadFileToBackend, generateChallengeQuestions } from './api'; 
import React, { useState } from 'react';
import { Bot, Sparkles, Brain, FileText } from 'lucide-react';
import FileUpload from './components/FileUpload';
import SummaryDisplay from './components/SummaryDisplay';
import ChallengeQuestions, { Question } from './components/ChallengeQuestions';
...
function App() {
  ...
  const handleFileUpload = async (file: File, content: string) => {
    ...
    try {
      ...
      setSummary(response.data.summary); // ✅ set the summary from response
    } catch (error) {
  ...
  const generateSummary = async (content: string, fileName: string) => {
    ...
    try {
      const summaryFromAPI = await uploadFileToBackend(uploadedFile as File);
      setSummary(summaryFromAPI);
    } catch (error) {
      console.error("Summary generation failed:", error);
      setSummary("An error occurred while generating summary.");
    }
  ...
  const generateQuestions = async () => {
    ...
    try {
      const questionsFromAPI = await generateChallengeQuestions(fileContent);
      // Handle both array and { questions: [...] }
      if (questionsFromAPI && Array.isArray(questionsFromAPI.questions)) {
        setQuestions(questionsFromAPI.questions);
